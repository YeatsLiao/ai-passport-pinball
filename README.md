...
| 顶部再入车道 ×3 | 2000 | §2.2 `roll1/2/3` |
| 倍率目标 ×3（左侧） | 单个 500；三个全倒 1500 并倍率升档 | §2.3 `target7/8/9` |
| 黑洞（两挡板之间的落球口） | 20000，吐球后冷却 6.0s | §2.4 `a_kout3` |
| 引力井（左上窄道） | 50000，冷却 8.0s | §2.4 `a_kout1` |
| 超空间洞（右上发射道旁） | 10000 / 20000 / 50000 / 150000，按灯环档位取，满 4 清环，冷却 3.0s | §2.4 `a_kout2` |

- **倍率**：实际入账 = base × 倍率，档位 x1/x2/x3/x5/x10，由倍率目标组完成推进，**每颗球结束归 x1**（§4.2）。
- **bumper 升级**：挥挡板点亮中央三盏升级灯，满 3 盏后任一车道**由灭变亮** → 清组 + 攻击档位 +1（RC 5 "Weapons Upgraded"）。
